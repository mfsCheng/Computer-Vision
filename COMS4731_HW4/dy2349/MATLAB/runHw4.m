...
close all;
src_pts_nx2 = points(1:4,:);
dest_pts_nx2 = points(5:8,:);
...
close all;
bg_pts = points(1:4,:);
portrait_pts = points(5:8,:);
%portrait_pts = [xp1 yp1; xp2 yp2; xp3 yp3; xp4 yp4];
%bg_pts = [xb1 yb1; xb2 yb2; xb3 yb3; xb4 yb4];
% bg_pts = [100.378787878788,17.4898989898989;84.8636363636363,437.691919191919;285.267676767677,423.469696969697;276.217171717172,71.7929292929292];
% portrait_pts = [0.994954591321971,0.703329969727406;1.65489404641778,399.306760847629;327.005045408678,399.306760847629;327.005045408678,0.703329969727406];
...
ransac_n = 15; % Max number of iteractions
...
% Your own panorama
imgc = im2single(imread('CU_center.png'));
imgl = im2single(imread('CU_left.png'));
imgr = im2single(imread('CU_right.png'));
stitched_img = stitchImg(imgc, imgl, imgr);
imwrite(stitched_img, 'CU_panorama.png');
